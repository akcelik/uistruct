  ...
  },
  {
    ...
    components: [
      ...
      },
      {
        ...
        title: 'Tree',
        selector: 'strct-tree',
        importNames: ['StrctTree', 'StrctTreeNode'],
        summary: 'Nested, expandable nodes with icons.',
        lead: 'Nested, expandable nodes with optional icons and active state. Compose `strct-tree-node` recursively.',
        inputs: [
          { name: 'label', type: 'string', description: 'On `strct-tree-node`: node text. Required.' },
        ...
        outputs: [
          { name: 'activated', type: 'void', description: 'On `strct-tree-node`: fired when the node is clicked.' },
        ],
        do: ['Use for hierarchical navigation (workspaces, file systems, inventory).'],
      ...
      },
      {
        ...
        lead: 'A multi-step flow with built-in Back / Next / Finish controls. Each `strct-step` carries a `label`; the wizard emits `finished` on completion.',
        inputs: [
        ...
        do: ['Use for a linear, sequential task with a clear end.'],
    ...
    components: [
      {
        ...
        importNames: ['StrctTable', 'StrctColumn'],
        summary: 'Lightweight styled table.',
        lead: 'A lightweight, token-styled table for simple tabular data. Pass `columns` and `rows`; values are looked up by column `key`.',
        inputs: [
      ...
      },
      {
        ...
        importNames: ['StrctDatagrid', 'StrctDatagridColumn'],
        summary: 'Selectable, expandable data grid.',
        lead: 'A richer grid with sorting, selection, batch actions, pagination and expandable rows. Columns can be `sortable` and aligned.',
        inputs: [
  ...
  },
  {
    ...
    components: [
      ...
      },
      {
        ...
        importNames: ['StrctContextMenu', 'StrctSubmenu'],
        summary: 'Right-click menu with submenus.',
        lead: 'A right-click context menu. Wrap the trigger area; project entries through `strctContextMenuItems` (reusing `strct-dropdown-item`), and nest `strct-submenu` for fly-outs.',
        inputs: [
          { name: 'label', type: 'string', default: `''`, description: 'On `strct-submenu`: the submenu row label.' },
          { name: 'icon', type: 'string', default: `''`, description: 'On `strct-submenu`: optional leading icon (intentionally omitted on pure submenus).' },
        ],
        methods: [
          { name: 'close()', type: '() => void', description: 'Close the menu programmatically.' },
        ],
        do: ['Use for power-user, right-click affordances that duplicate primary actions.'],
        dont: ['Do not hide the only way to perform an action behind a right-click menu.'],
        a11y: ['Opens at the cursor, stays inside the viewport, and closes on click / Escape / resize.'],
      },
    ],
  },
];
