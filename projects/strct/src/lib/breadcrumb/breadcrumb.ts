...
@Component({
  ...
  styles: [
    `
    .strct-bc { display: flex; align-items: center; flex-wrap: wrap; gap: 2px; font-size: 13px; }
    .strct-bc__item:not(:last-child)::after {
      content: '/'; margin: 0 8px; color: var(--t3); font-weight: 400;
    }
    `,
  ],
})
...
@Component({
  ...
  styles: [
    `
    .strct-bc__item { display: inline-flex; align-items: center; color: var(--t2); }
    .strct-bc__item a { color: var(--t2); text-decoration: none; }
    .strct-bc__item a:hover { color: var(--acc); text-decoration: none; }
    .strct-bc__item--current { color: var(--t1); font-weight: 600; }
    `,
  ],
})
