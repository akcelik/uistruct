...
@Component({
  ...
  styles: [
    `
    .strct-gauge { display: inline-block; }
    .strct-gauge__wrap { position: relative; }
    .strct-gauge__track { stroke: var(--bg-3); }
    .strct-gauge__value { transition: stroke-dasharray .4s ease; }
    .strct-gauge__center {
      position: absolute; inset: 0; display: flex; flex-direction: column;
      align-items: center; justify-content: center; pointer-events: none;
    }
    .strct-gauge__num { font-size: 22px; font-weight: 600; color: var(--t1); line-height: 1; }
    .strct-gauge__pct { font-size: 12px; color: var(--t2); margin-left: 1px; }
    .strct-gauge__label { font-size: 11px; color: var(--t2); margin-top: 3px; }
    `,
  ],
})
